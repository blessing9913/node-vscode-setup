...
function solveAFunctional() {
  const cities = people.filter(({ age }) => age < 30).map(({ city }) => city)
  ...
  for (const person of people) {
    ...
    if (petOrPets) {
      ...
      if (typeof petOrPets === 'string') {
        const pet = petOrPets
        const origNumPetsCity = petsOfCity[pet] || 0
        petsOfCity[pet] = origNumPetsCity + 1
      } else {
